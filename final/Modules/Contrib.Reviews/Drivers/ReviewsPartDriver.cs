...
namespace Contrib.Reviews.Drivers {
    ...
    public class ReviewsPartDriver : ContentPartDriver<ReviewsPart> {
        ...
        private ReviewsPart BuildStars(ReviewsPart part, string displayType) {
            part.Rating.CurrentVotingResult = _votingService.GetResults(part.ContentItem.Id).Where(result => result != null && result.FunctionName == "average").FirstOrDefault() ?? new ResultRecord();
